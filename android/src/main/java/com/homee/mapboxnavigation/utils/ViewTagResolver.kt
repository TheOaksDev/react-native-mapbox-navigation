package com.homee.mapboxnavigation.utils
...
open class ViewTagResolver(val context: ReactApplicationContext) {
    ...
    private val manager : UIManager
        get() = UIManagerHelper.getUIManager(context, UIManagerType.DEFAULT)!!
        ...
        context.runOnUiQueueThread() {
            try {
                ...
                if (view != null) {
                    ...
                    Log.e(LOG_TAG, "view: $resolvedView found with tag: $viewTag but it's either null or not the correct type")
                ...
                if (!createdViews.contains(viewTag)) {
                    viewWaiters.getOrPut(viewTag) { mutableListOf<ViewTagWaiter<View?>>() }.add(ViewTagWaiter<View?>({ view ->
                        if (view != null) {
                            ...
                            Log.e(LOG_TAG, "view: $viewTag but is null")
